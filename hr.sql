...
references HR.employee(emp_entity_id) ON DELETE CASCADE ON UPDATE CASCADE;
alter table HR.employee_pay_history add constraint pk_ephi_rate_change_date primary key(ephi_entity_id,ephi_rate_change_date);
...
references HR.employee(emp_entity_id) ON DELETE CASCADE ON UPDATE CASCADE;
alter table HR.employee_client_contract add constraint fk_ecco_account_manager foreign key (ecco_account_manager) references HR.employee(emp_entity_id);
...
alter table HR.employee add constraint fk_emp_entity_id foreign key (emp_entity_id) references users.users(user_entity_id);
alter table HR.employee add constraint fk_emp_joro_id foreign key (emp_joro_id) references master.job_role(joro_id);
alter table HR.employee_client_contract add constraint fk_ecco_clit_id foreign key (ecco_clit_id) references job_hire.client(clit_id);
alter table HR.employee_client_contract add constraint fk_ecco_joty_id foreign key (ecco_joty_id) references master.job_type(joty_id);
alter table HR.employee_client_contract add constraint fk_ecco_status foreign key (ecco_status) references master.status(status);
